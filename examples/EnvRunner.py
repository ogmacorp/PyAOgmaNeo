...
class EnvRunner:
    def __init__(self, env, layerSizes=3 * [ (4, 4, 32) ], layerRadius=2, hiddenSize=(8, 8, 16), imageRadius=8, imageScale=1.0, obsResolution=32, actionResolution=9, rewardScale=1.0, terminalReward=0.0, infSensitivity=3.0, nThreads=4):
        self.env = env
        ...
        for i in range(len(self.actionIndices)):
            ...
            self.h.setInputImportance(index, 0.0)
    ...
    def act(self, epsilon=0.0, obsPreprocess=None):
        ...
        obs, reward, term, trunc, info = self.env.step(feedActions)
