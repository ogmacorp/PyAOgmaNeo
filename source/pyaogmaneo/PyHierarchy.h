...
public:
    ...
    void setEEta(
        ...
        if (eta < 0.0f) {
            std::cerr << "Error: EEta must be >= 0.0" << std::endl;
            abort();
        }

        h.getELayer(l).eta = eta;
    }
    ...
    void setADecay(
        ...
        if (decay < 0.0f) {
            std::cerr << "Error: ADecay must be >= 0.0" << std::endl;
            abort();
        }

        h.getALayer(i).decay = decay;
    }
