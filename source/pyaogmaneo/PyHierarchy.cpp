...
void Hierarchy::step(
    ...
    if (inputCIs.size() != h.getInputSizes().size()) {
        std::cerr << "Incorrect number of inputCIs passed to step! Received " << inputCIs.size() << ", need " << h.getInputSizes().size() << std::endl;
        abort();
    }
    ...
    for (int i = 0; i < inputCIs.size(); i++) {
        int numColumns = h.getInputSizes()[i].x * h.getInputSizes()[i].y;
        ...
        for (int j = 0; j < inputCIs[i].size(); j++) {
            if (inputCIs[i][j] < 0 || inputCIs[i][j] >= h.getInputSizes()[i].z) {
                std::cerr << "Input CSDR at input index " << i << " has an out-of-bounds column index (" << inputCIs[i][j] << ") at column index " << j << ". It must be in the range [0, " << (h.getInputSizes()[i].z - 1) << "]" << std::endl;
                abort();
            }
...
std::vector<int> Hierarchy::getPredictionCIs(
    ...
    if (i < 0 || i >= h.getInputSizes().size()) {
        std::cout << "Prediction index " << i << " out of range [0, " << (h.getInputSizes().size() - 1) << "]!" << std::endl;
        abort();
    }
